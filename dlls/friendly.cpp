	...
	if(pev->deadflag == DEAD_NO ){
		...
		this->m_pSchedule == slRangeAttack1 //only if attacking. Other uses of the anim don't count.
			...
			if(rapidVomitCheck == FALSE){
				...
				rapidVomitCheck_ScheduleFinish = FALSE;
				nextNormalThinkTime = 0;  //do think logic this time, as it was scheduled this frame. We weren't in rapidVomitCheck mode before.
			}
...
int CFriendly::LookupActivityHard(int activity){
	...
	switch(activity){
		...
		case ACT_RUN:
			...
			if(::g_iSkillLevel == SKILL_HARD) this->m_flFramerateSuggestion *=1.04;
			return FRIENDLY_WALK;
		break;
