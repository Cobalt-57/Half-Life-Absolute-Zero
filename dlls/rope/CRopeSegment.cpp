...
void CRopeSegment::Precache()
{
	...
	PRECACHE_MODEL( (char*)STRING(m_iszModelName) );
	PRECACHE_SOUND( "items/rope/grab_rope.wav" );
}
...
void CRopeSegment::Spawn()
{
	...
	SET_MODEL(ENT(pev), (char*)STRING(m_iszModelName));
	...
	if( pOther->IsPlayer() )
	{
		...
		if( m_pSample->GetMasterRope()->IsAcceptingAttachment() && !pPlayer->IsOnRope() )
		{
			if( m_bCanBeGrabbed )
			{
				...
				{
					EMIT_SOUND( ENT(pev), CHAN_BODY, "items/rope/grab_rope.wav", 1.0, ATTN_NORM );
				}
			}
			else
			{
...
{
	CRopeSegment* pSegment = (CRopeSegment*)( UTIL_CreateNamedEntity( "rope_segment" ) );
