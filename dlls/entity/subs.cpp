...
void CBaseDelay::PostRestore(){
	//easyForcePrintLine("PostRestore: CBaseDelay: %s", getClassname());
}
...
//Cloned method, put more printouts and have some place call this instead of the normal one above..
void FireTargetsTest( const char *targetName, CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	edict_t *pentTarget = NULL;
	if ( !targetName )
		return;
...
//MODDD - SetMoveDir moved to util.h/.cpp
...
void CBaseToggle::PostRestore(){
	//easyForcePrintLine("PostRestore: CBaseToggle: %s", getClassname());
}
