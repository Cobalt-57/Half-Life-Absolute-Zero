...
void CNodeEnt :: Spawn( void )
{
	...
	if (FClassnameIs( pev, "info_node_air" )){
		...
		//MODDD - yes?
		WorldGraph.map_anyAirNodes = TRUE;
	...
	for ( i = 0 ; i < WorldGraph.m_cNodes ; i++ )
	{
		...
		for ( j = 0 ; j < pSrcNode->m_cNumLinks ; j++ )
		{
			...
			for ( hull = 0 ; hull < MAX_NODE_HULLS; hull++ )
			{
				...
				if(WorldGraph.node_hulltest_heightswap != TRUE){
				...
				if(pHullTest_SrcNode->m_afNodeInfo & bits_NODE_LAND){
					hulltestExtraVertical = WorldGraph.node_hulltest_height;
				}else{
...
void CTestHull :: PathFind ( void )
{
	...
	for ( i = 0 ; i < iPathSize - 1 ; i++ )
	{
		...
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
			...
			WRITE_COORD( pNode->m_vecOrigin.z + WorldGraph.node_linktest_height );

			WRITE_COORD( pNextNode->m_vecOrigin.x);
			WRITE_COORD( pNextNode->m_vecOrigin.y);
			WRITE_COORD( pNextNode->m_vecOrigin.z + WorldGraph.node_linktest_height);
		MESSAGE_END();
...
void CNodeViewer :: DrawThink( void )
{
	...
	while(TRUE)
	{
		...
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
			...
			WRITE_COORD( WorldGraph.m_pNodes[ m_aFrom[m_iDraw] ].m_vecOrigin.z + WorldGraph.node_linktest_height );

			WRITE_COORD( WorldGraph.m_pNodes[ m_aTo[m_iDraw] ].m_vecOrigin.x );
			WRITE_COORD( WorldGraph.m_pNodes[ m_aTo[m_iDraw] ].m_vecOrigin.y );
			WRITE_COORD( WorldGraph.m_pNodes[ m_aTo[m_iDraw] ].m_vecOrigin.z + WorldGraph.node_linktest_height );
