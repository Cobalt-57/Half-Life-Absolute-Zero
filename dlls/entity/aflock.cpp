...
//MODDD NOTE - interestingly enough this monster does not inherit from CFlyer, flyer.h & .cpp.
...
#if EXTRA_NAMES > 0
	LINK_ENTITY_TO_CLASS( flyer, CFlockingFlyer );
	LINK_ENTITY_TO_CLASS( flyer_flock, CFlockingFlyerFlock );

	#if EXTRA_NAMES == 2
		LINK_ENTITY_TO_CLASS( alien_flyer, CFlockingFlyer );
		LINK_ENTITY_TO_CLASS( alien_flyer_flock, CFlockingFlyerFlock );
		LINK_ENTITY_TO_CLASS( monster_alien_flyer, CFlockingFlyer );
		LINK_ENTITY_TO_CLASS( monster_alien_flyer_flock, CFlockingFlyerFlock );
	#endif
...
void CFlockingFlyerFlock :: Spawn( )
{
	...
	pev->classname = MAKE_STRING("monster_flyer_flock");
...
//=========================================================
//=========================================================
GENERATE_KILLED_IMPLEMENTATION(CFlockingFlyer)
{
...
void CFlockingFlyer :: SquadRemove( CFlockingFlyer *pRemove )
{
	...
	if ( SquadCount() > 2 )
	{
		...
		if ( pRemove == this )
		{
			...
			pLeader->setEnemyLKP(m_vecEnemyLKP);
