...
BOOL CEnvGlobal::IsWorldAffiliated(){
	return TRUE;
}
...
BOOL CMultiSource::IsWorldAffiliated(){
	return TRUE;
}
...
BOOL CBaseButton::IsWorldAffiliated(){
	return TRUE;
}
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CBaseButton)
{
	GENERATE_TRACEATTACK_PARENT_CALL(CBaseToggle);
}

GENERATE_TAKEDAMAGE_IMPLEMENTATION(CBaseButton)
{
...
//MODDD - accept another var too.
void DoSpark(entvars_t *pev, const Vector &location, float arg_sparksMod )
{
...
void DoSpark(entvars_t *pev, const Vector &location){
	DoSpark(pev, location, EASY_CVAR_GET_DEBUGONLY(sparksEnvMulti));
}
...
void CBaseButton::ButtonSpark ( void )
{
	...
	DoSpark( pev, pev->mins, EASY_CVAR_GET_DEBUGONLY(sparksButtonMulti) );
}
...
//MODDD - new
CMomentaryRotButton::CMomentaryRotButton(void){
	//default: no-go.
	m_fAngleTrigger = -1;
}
...
void CMomentaryRotButton::KeyValue( KeyValueData *pkvd )
{
	...
	//MODDD - new
	else if (FStrEq(pkvd->szKeyName, "m_fAngleTrigger"))
	{
		m_fAngleTrigger = atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
		CBaseToggle::KeyValue( pkvd );
...
BOOL CMomentaryRotButton::IsWorldAffiliated(){
	return TRUE;
}
...
void CMomentaryRotButton::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( FBitSet(pev->spawnflags, SF_MOMENTARY_ROTATING_REQUIREMASTERTRIGGER) && !UTIL_IsMasterTriggered(m_sMaster, m_hActivator))
	{
		// button is locked, play locked sound
		//easyForcePrintLine("NOT UTIL_IsMasterTriggered");
		//PlayLockSounds(pev, &m_ls, TRUE, TRUE);
		return;
	}else{
		//easyForcePrintLine("YES UTIL_IsMasterTriggered");
...
BOOL CEnvSpark::IsWorldAffiliated(){
	return TRUE;
}
...
void EXPORT CEnvSpark::SparkThink(void)
{
	...
	if(pev->spawnflags & SF_ENVSPARK_REDUCEDSPARKS){
		...
		DoSpark( pev, pev->origin, EASY_CVAR_GET_DEBUGONLY(sparksEnvMulti) * 0.5 );
	}else{
		DoSpark( pev, pev->origin, EASY_CVAR_GET_DEBUGONLY(sparksEnvMulti) );
	}
...
BOOL CButtonTarget::IsWorldAffiliated(){
	return TRUE;
}
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CButtonTarget)
{
	GENERATE_TRACEATTACK_PARENT_CALL(CBaseEntity);
}

GENERATE_TAKEDAMAGE_IMPLEMENTATION(CButtonTarget)
{
