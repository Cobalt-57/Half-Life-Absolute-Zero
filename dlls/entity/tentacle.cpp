...
void CTentacle :: Spawn( )
{
	...
	pev->classname = MAKE_STRING("monster_tentacle");
...
//Finally, the dev's made a constructor for once!
CTentacle::CTentacle( )
{
	m_flMaxYaw = 65;
	m_iTapSound = 0;
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CTentacle)
{
	GENERATE_TRACEATTACK_PARENT_CALL(CBaseMonster);
}

GENERATE_TAKEDAMAGE_IMPLEMENTATION(CTentacle)
{
...
GENERATE_KILLED_IMPLEMENTATION(CTentacle)
{
	m_iGoalAnim = TENTACLE_ANIM_Pit_Idle;
	return;
}
...
//MODDD NOTE - Looks like this monster goes completely unused. Especially since the model it looks for, "models/maw.mdl" does not exist at all.
/*
...
#if EXTRA_NAMES > 0
	LINK_ENTITY_TO_CLASS(tentaclemaw, CTentacleMaw );
...
void CTentacleMaw :: Spawn( )
{
	...
	pev->classname = MAKE_STRING("monster_tentaclemaw");
