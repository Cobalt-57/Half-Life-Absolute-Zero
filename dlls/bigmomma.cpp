...
public:
	...
	void SetObjectCollisionBox( void )
	{
		if(pev->deadflag != DEAD_NO){
			pev->absmin = pev->origin + Vector( -176, -176, 0 );
			pev->absmax = pev->origin + Vector( 176, 176, 190 );
		}else{
...
private:
	...
	#if EXTRA_NAMES > 0
		LINK_ENTITY_TO_CLASS( bigmomma, CBigMomma );
		LINK_ENTITY_TO_CLASS( gonarch, CBigMomma );

	#if EXTRA_NAMES == 2
		LINK_ENTITY_TO_CLASS( monster_gonarch, CBigMomma );
	#endif
...
void CBigMomma :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{
		...
		case BIG_AE_ATTACK_SOUND:
			EMIT_SOUND_ARRAY_FILTERED( CHAN_WEAPON, pAttackSounds );
			break;

		case BIG_AE_BIRTH_SOUND:
			EMIT_SOUND_ARRAY_FILTERED( CHAN_BODY, pBirthSounds );
			break;

		case BIG_AE_SACK:
			if ( RANDOM_LONG(0,100) < 30 )
				EMIT_SOUND_ARRAY_FILTERED( CHAN_BODY, pSackSounds );
			break;
		...
		case BIG_AE_STEP3:		// Footstep back left
			EMIT_SOUND_ARRAY_FILTERED( CHAN_ITEM, pFootSounds );
			break;

		case BIG_AE_STEP4:		// Footstep back right
		case BIG_AE_STEP2:		// Footstep right
			EMIT_SOUND_ARRAY_FILTERED( CHAN_BODY, pFootSounds );
			break;
...
GENERATE_TRACEATTACK_IMPLEMENTATION(CBigMomma)
{
	if ( ptr->iHitgroup != 1 )
	{
		// didn't hit the sack?
...
GENERATE_TAKEDAMAGE_IMPLEMENTATION(CBigMomma)
{
...
void CBigMomma::DeathNotice( entvars_t *pevChild )
{
	...
	if ( IsAlive() )
	{
		...
		EMIT_SOUND_ARRAY_FILTERED( CHAN_WEAPON, pChildDieSounds );
	}
}
...
void CBigMomma :: Spawn()
{
	...
	//How does collision with other parts still work??
...
void CBigMomma :: Precache()
{
	...
	//MODDD - IMPORTANT: turn this back on after doing "precacheOther", since precacheOther will turn it off when it is done.
	global_useSentenceSave = TRUE;
...
//MODDD - for a test as seen below.  This is cheaper than an instance var (programming-wise).
//int lastSecond = 0;
...
void CBigMomma::RunTask( Task_t *pTask )
{
	...
	case TASK_DIE:
		...
		if ( m_fSequenceFinished && pev->frame >= 255 )
		{
			...
			//easyPrintLine("gonarch deadbox drawn");
	...
	/*
	//this section borrowed from "barney".
