...
//MODDDMIRRROR - pretty significant.
...
BOOL CEnvMirror::canDrawPlayer(void){
	...
	return TRUE; //let's leave this up to CVars and not the mirror itself, which are made to bypass this if they don't allow something.
	
	//Space is precious in sending flags over to rendering anways (which would need to know what mirrors should deny rendering the player's model in third person which happens regardless of this marker being on or off, whether that matters or not).
	
	//Typical way, checking a spawnflag that our mapmakers may not even be aware of for mirrors:
