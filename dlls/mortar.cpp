...
#if 0
	int pitch = RANDOM_LONG(95,124);
	EMIT_SOUND_FILTERED(ENT(pev), CHAN_VOICE, "weapons/mortarhit.wav", 1.0, 0.55, 0, pitch, TRUE);

	// ForceSound( SNDRADIUS_MP5, bits_SOUND_COMBAT );

	// ExplodeModel( pev->origin, 400, g_sModelIndexShrapnel, 30 );

	RadiusDamageAutoRadius ( pev, VARS(pev->owner), pev->dmg, CLASS_NONE, DMG_BLAST );
