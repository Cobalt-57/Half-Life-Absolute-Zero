...
/*
//MODDD - REVOKED
//MODDD - copied and pasted in hgrunt.cpp to check for hassault as well (TODO!!!)
int CSquadMonster :: SquadRecruit( int searchRadius, int maxMembers )
{
	int squadCount;
	int iMyClass = Classify();// cache this monster's class
	...
	if ( !FStringNull( pev->netname ) )
	{
		...
		while ( pEntity )
		{
			...
			if ( pRecruit )
			{
				if ( !pRecruit->InSquad() && pRecruit->Classify() == iMyClass && pRecruit != this && pRecruit->m_afCapability & bits_CAP_SQUAD )
				{
					// minimum protection here against user error.in worldcraft. 
					if (!SquadAdd( pRecruit )){
						if(canPrint){
	...
	else 
	{
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, searchRadius )) != NULL)
		{
			...
			BOOL canPrint = FALSE;
			if(pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine && !pRecruit->InSquad() && pRecruit->Classify() == iMyClass &&
				   ( (iMyClass != CLASS_ALIEN_MONSTER) || FStrEq(STRING(pev->classname), STRING(pRecruit->pev->classname))) &&
				    FStringNull( pRecruit->pev->netname ) ){
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine && pRecruit->m_afCapability & bits_CAP_SQUAD )
			{
				// Can we recruit this guy?
...
void CSquadMonster :: StartMonster( void )
{
	...
	if ( ( m_afCapability & bits_CAP_SQUAD ) && !InSquad() )
	{
		...
		//if ( IsLeader() && FClassnameIs ( pev, "monster_human_grunt" ) && this->SquadCount() >= 3 )
		...
		if ( FClassnameIs ( pev, "monster_human_grunt" ) && testLeader != NULL && !FClassnameIs(testLeader->pev, "monster_human_assault") )
		{
			...
			pReplacement = Instance( pent );
			pReplacement->pev->owner = NULL;
			pReplacement->pev->origin = testLeader->pev->origin;
			pReplacement->pev->angles = testLeader->pev->angles;
		...
		}//END OF if(FClassNameIs(monster_human_grunt)... )
