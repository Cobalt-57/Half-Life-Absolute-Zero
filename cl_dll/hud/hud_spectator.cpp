...
void SpectatorSpray(void)
{
	...
	VectorAdd_f(forward, v_origin, forward);
...
void CHudSpectator::SetSpectatorStartPosition()
{
	...
	else
	{
		...
		VectorCopy_f(vec3_origin, m_cameraOrigin);
		VectorCopy_f(vec3_origin, m_cameraAngles);
	}
	
	VectorCopy_f(m_cameraOrigin, vJumpOrigin);
	VectorCopy_f(m_cameraAngles, vJumpAngles);
...
int CHudSpectator::Draw(float flTime)
{
	...
	if ( (m_moveDelta != 0.0f) && (g_iUser1 != OBS_ROAMING) )
	{
		...
		VectorAdd_f(m_mapOrigin, right, m_mapOrigin);
...
void CHudSpectator::FindNextPlayer(bool bReverse)
{
	...
	else
	{
		...
		VectorCopy_f ( pEnt->origin, vJumpOrigin );
		VectorCopy_f ( pEnt->angles, vJumpAngles );
	}
	iJumpSpectator = 1;
}
...
void CHudSpectator::SetModes(int iNewMainMode, int iNewInsetMode)
{
	...
	if ( iNewMainMode != g_iUser1 )
	{
		...
		if ( (g_iUser1 == OBS_IN_EYE) || (g_iUser1 == OBS_ROAMING) ) 
		{
			...
			//MODDD - all "SetCrosshair" calls have been changed to "SetCrosshairFiltered".  See cl_util.h and util.cpp, client-side.
			SetCrosshairFiltered( m_hCrosshair, m_crosshairRect, 255, 255, 255 );
		}
		else
		{
...
void CHudSpectator::DrawOverviewEntities()
{
	...
	for (i=0 ; i < MAX_OVERVIEW_ENTITIES ; i++)
	{
		...
		VectorSubtract_f(offset, screen, offset );
	...
	{
		VectorCopy_f( v_sim_org, origin );
		VectorCopy_f( v_cl_angles, angles );
	}
	else
