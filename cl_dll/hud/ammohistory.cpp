...
int HistoryResource :: DrawAmmoHistory( float flTime )
{
	for ( int i = 0; i < MAX_HISTORY; i++ )
	{
		if ( rgAmmoHistory[i].type )
		{
			...
			{
				wrect_t rcPic;
				SpriteHandle_t *spr = gHUD.m_Ammo.gWR.GetAmmoPicFromWeapon( rgAmmoHistory[i].iId, rcPic );
			...
			{
				WEAPON *weap = gHUD.m_Ammo.gWR.GetWeapon( rgAmmoHistory[i].iId );
				...
				if ( !gHUD.m_Ammo.gWR.HasAmmo( weap ) )
