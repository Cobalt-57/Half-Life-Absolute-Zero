...
//MODDD - anything related to classes removed.
// Most things related to teams can probably be removed but best to be safe for now, I'm not very familiar with HL multiplayer so
// I'm not sure if/what I would be breaking.
	...
	try
	{
		...
		while ((strlen(token) > 0) && (m_iNumMenus < MAX_MENUS))
		{
			...
			while (token[0] != '}' && (strlen(token) > 0))
			{
				...
				{
					//MODDD - only something class-related, so no.
				}
				...
				else
				{
					// normal button
					//MODDD - no class
					pButton = new CommandButton(cText, xOffset, iButtonY, flButtonSizeX, flButtonSizeY, false, flatDesign);
				}
...
CommandButton* TeamFortressViewport::CreateCustomButton(char* pButtonText, char* pButtonName, int iYOffset)
{
	...
	// ChangeTeam
	//MODDD - doubt HL really supports this but eh, wipe later
	if (!strcmp(pButtonName, "!CHANGETEAM"))
	{
		// ChangeTeam Submenu
		pButton = new CommandButton(pButtonText, 0, BUTTON_SIZE_Y * 2, CMENU_SIZE_X, BUTTON_SIZE_Y, false, false);
	...
	{
		//MODDD - why was this using a ClassButton?  Deleted so using CommandButton instead
		pButton = new CommandButton(pButtonText, 0, BUTTON_SIZE_Y * m_pCurrentCommandMenu->GetNumButtons(), CMENU_SIZE_X, BUTTON_SIZE_Y, false, false);
...
CMenuPanel* TeamFortressViewport::CreateTextWindow(int iTextToShow)
{
	...
	if (iTextToShow == SHOW_MOTD)
	{
		if (!m_szServerName || !m_szServerName[0])
			strcpy(cTitle, GAME_NORMAL_DESCRIPTION);
		else
	...
	//MODDD - replaced class-check with TRUE.
	CMenuPanel* pMOTDPanel = CMessageWindowPanel_Create(cText, cTitle, TRUE, false, 0, 0, ScreenWidth, ScreenHeight);
...
int TeamFortressViewport::MsgFunc_ScoreInfo(const char* pszName, int iSize, void* pbuf)
{
	...
	//short playerclass = READ_SHORT();   //MODDD - removed, no longer expected.
