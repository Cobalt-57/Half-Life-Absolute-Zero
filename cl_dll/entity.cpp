...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	//baseline
	//easyPrintLine("WHAT IS MODEL!!!!!!!!!!!! %d", entity->curstate.effects);

	//easyPrintLine("WHAT IS MODEL %s", entity->curstate.gamest);
	//easyPrintLine("WHAT IS MODELEEE %d", entity->baseline.curstate);
	//easyPrintLine("WHAT IS MODEL %.2f", entity->baseline.fuser1);
	//easyPrintLine("WHAT IS MODEL ER %d", entity->curstate.renderfx);
	//entity->index
	//cl_entities/???

	//entity->curstate.;

	//The model should be 287 when the glock fires!
	//easyPrintLine("WHAT IS MODEL %d", entity->curstate.eflags);
	//easyPrintLine("WHAT IS MODEL %d", entity->curstate.iuser1);
	
	//easyPrintLine("MODDELLL: %s", entity->model);
	
	//easyPrintLine("What? %d", entity->curstate.iuser1 );

	if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(eventsAreFabulous) == 1){
	...
	/*
	easyForcePrintLine("ROCK N ROLL GANGSTER %d %d %d %d %.2f %.2f %.2f %.2f",
		entity->curstate.iuser1,
		entity->curstate.iuser2,
		entity->curstate.iuser3,
		entity->curstate.iuser4,
		entity->curstate.fuser1,
		entity->curstate.fuser2,
		entity->curstate.fuser3,
		entity->curstate.fuser4
	);
	*/
...
//(entity->curstate.renderfx & ISVIEWMODEL) == ISVIEWMODEL
	...
	if(event->event == 5001 || event->event == 5011 || event->event == 5021 || event->event == 5031){
		...
		if((entity->curstate.renderfx & NOMUZZLEFLASH) == NOMUZZLEFLASH /*|| entity->curstate.iuser2 == 200*/){
			//easyForcePrintLine("HOLY ahhh %d : %d", entity->curstate.renderfx, entity->curstate.iuser2);
			return;
		}
	...
	case 5001:
		...
		//MODDD - extra condition.  If this is the player's glock (iuser1 being -500),  and the body is 1 (silencer), do not show 
		//the muzzle flash.

		//easyPrintLine("HOW DO I DO? %d %d %d ", (entity->curstate.renderfx), entity->curstate.iuser1, entity->curstate.body);
		//"160" = "ISVIEWMODEL" is a special combo for the player only.  If so, we can still render, other criteria is here for stopping the flash.  (Do we even need the "128" criteria at all anymore?  Can't see a difference).
		


		
		//if( ( (entity->curstate.renderfx & ISVIEWMODEL)==ISVIEWMODEL || !(entity->curstate.renderfx & ISPLAYER))  && (entity->curstate.iuser1 != -500 || entity->curstate.body == 0) ){
			//easyForcePrintLine("muzzle flash from 5001??");
	...
	case 5011:
		...
		//MODDD - extra condition.  Managed to hijack the "renderfx" variable to include a new bit:  silencer block.  If 128 is "on", do not show
		//the muzzle effect.
	...
	case 5021:
		...
		if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(event5021Allowed) == 0){
			return;
		}
		gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[2], atoi( event->options) );
		break;
	case 5031:
		if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(event5031Allowed) == 0){
			return;
		}
		gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&entity->attachment[3], atoi( event->options) );
		break;
	case 5002:
		if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(event5002Allowed) == 0){
			return;
		}
	...
	case 5004:
		//easyForcePrintLine("HUD_StudioEvent, 5004 extra info: %s : can?%.1f", (char*)event->options, EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(event5004Allowed));
		if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(event5004Allowed) == 0){
			return;
		}
	...
	void ( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	//easyPrintLine("WHAT FRA %d", gTempEntFrame);
	...
	while ( pTemp )
	{
		...
		else
		{
			...
			//easyPrintLine("WHAT FLAGZ %d", pTemp->flags);
	...
	//easyForcePrintLine("WOOOOOOOOOOO %.2f", pTemp->entity.curstate.framerate);
...
// Experiment
			...
			{
				//easyPrintLine("CRAZY ANGLES: %.2f %.2f %.2f", pTemp->entity.baseline.angles[0], pTemp->entity.baseline.angles[1], pTemp->entity.baseline.angles[2]);
			...
			if ( pTemp->flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD) )
			{
				...
				if ( traceFraction != 1 )	// Decent collision now, and damping works
				{
					...
					//easyPrintLine("BOUNCE FACTOR: %.2f", pTemp->bounceFactor);
					...
					if(pTemp->flags & FTENT_FLASHONHIT){
						//easyPrintLine("FLASHIN LIKE A BAWSS");
						//let's do this in ev_hldm instead by custom effects.
					}
					...
					if (pTemp->hitSound)
					{
						...
						if(EASY_CVAR_GET_CLIENTONLY_DEBUGONLY(muteTempEntityGroundHitSound) != 1){
			...
			if(EASY_CVAR_GET_CLIENTSENDOFF_BROADCAST_DEBUGONLY(trailTypeTest) != -1){
				...
				if ( (pTemp->flags & FTENT_ORANGETRAIL) )
				{
